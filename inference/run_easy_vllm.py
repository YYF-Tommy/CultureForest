...
def main(): 
    ...
    if think_mode:
        tgt_input_path = f"XXX/Easy/{model.split('/')[-1]}/input"
        tgt_output_path = f"XXX/Easy/{model.split('/')[-1]}/output"
    else:
        tgt_input_path = f"XXX/Easy/{model.split('/')[-1]}_nonThink/input"
        tgt_output_path = f"XXX/Easy/{model.split('/')[-1]}_nonThink/output"
    ...
    for folder in countryORregion_folders:
        ...
        for file in files:
            ...
            for line in data:
                ...
                complete_question = line["Easy"]["Question_MCQ"]
                messages = [
